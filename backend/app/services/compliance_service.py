...
def _map_to_engine_fields(data: ExtractedProductData) -> dict:
    ...
    fields = {
        ...
        "best_before": f(m.best_before if m.best_before is not None else ""),
...
def _map_product_context(data: ExtractedProductData) -> dict:
    ...
    is_food = (
        data.nutrition is not None 
        or (data.packaging and data.packaging.fssai_license_number)
        or (data.packaging and data.packaging.is_vegetarian is not None)
    )
...
def _passed_to_dict(v) -> dict:
    """Convert a passed-check Violation dataclass into a structured dict for the frontend."""
    return {
        "rule_id": v.rule_id,
        "rule_name": v.title,
        "severity": v.severity,
...
class ComplianceService:
    async def evaluate_rules(self, extracted_data: ExtractedProductData) -> dict[str, Any]:
        ...
        return {
            ...
            "passed_rules": [_passed_to_dict(v) for v in result.passed_checks],
